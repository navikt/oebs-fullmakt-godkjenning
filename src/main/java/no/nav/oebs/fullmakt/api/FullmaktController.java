...
public class FullmaktController {
    ...
    public String hentFullmakter(
            ...
            @Parameter(description = "f.eks. 159101", required = false) String segmentverdi,
            @RequestParam(name="segment", required = false)
            @Parameter(description = "f.eks. Kostnadssted", required = false) String segment,
            @RequestParam(name = "minBelopsgrense", required = false)
            @Parameter(description = "f.eks. 25000", required = false) Integer minBelopsgrense,
            @RequestParam(name="token", required = true)
            @Parameter(description = "tokenet fra Azure AD", required = true) String token) {

            // Fulmakter hentes fra Fullmaktregister
            return fullmaktservice.hentFullmaktService(segmentverdi, segment, minBelopsgrense, token);
    }
}
